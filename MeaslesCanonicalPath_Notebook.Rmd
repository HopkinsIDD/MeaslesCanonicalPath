...
for (c in 1:length(regs)){
  for (y in 1:3){
    ...
    all.plots[[name]] <- 
      ...
      scale_fill_distiller(palette="YlGnBu", direction=1, limits=c(0,0.25), na.value="white") + #Greys
      ylim(c(0, max(dfa$obs.exp) + 0.5)) +
      coord_polar(theta="x") + annotate("text", x = 0, y = 0, label = round(dfd[1,1],1)) +
...
```{r, echo = include.code, fig.height = 4, fig.width = 4}
    ...
    all.together <-  ggplot(dfa, aes(x=a, y=obs.exp, fill=(frequency))) +
      ...
      theme(panel.background=element_blank(),
            ...
            axis.text.x=element_blank(),
            axis.ticks=element_blank(),
            axis.text.y=element_text(size=5)) +  annotate("text", x = 0, y = 0, label = round(dfd[1,1],1)) +
...
mean.age.sus$Country = All.countries
...
for(i in 1 : length(All.countries)){
    for(j in 1 : length(years)){
        ...
        k = which(mean.age.sus$Country == All.countries[i])
