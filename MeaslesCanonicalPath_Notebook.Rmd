...
granular.canonical.path = calculate.granular.path(canonical.path, number.points = 200)
...
levelplot(as.matrix(prop.sus.by.age), ylab="age (years)", xlab="position on canonical path", 
          col.regions=colorRampPalette(brewer.pal(9,"Greys")), aspect=1/2, 
          scales=list(y=list(at=seq(2,60,2), labels=seq(1,59,2))))
...
ggplot(k, aes(x = year, y = weight, col = factor(w))) + geom_point(cex = 3) +
    ...
    scale_colour_manual(values=wes_palette("Darjeeling")[c(1,5)]) #potentially Darjeeling without the 1 pending R version
