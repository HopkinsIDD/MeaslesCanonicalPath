...
Fig. 2A shows the 38 point path (unscaled version), in which the grey band represents where the global population lies on the path currently (i.e., furthest position on the path by 2017). For comparison, we also displayed the global population position on the path based on year 1990, as seen in Fig. 2B of the paper.
...
#Furthest position for each country
df2017.tmp <- df.tmp %>%
  filter(Year>2012) %>%
  group_by(Country) %>%
...
population <- log(df2017.tmp$pop)+1
index <- match(1:38, df2017.tmp$closest)
...
#1990 position for each country
df1990.tmp <- df.tmp %>%
  group_by(Country) %>%
  filter(Year==1990) %>%
  group_by(closest) %>%
...
#Scaling and smoothing the population over the canonical path
population <- log(df1990.tmp$pop)+1
index <- match(1:38, df1990.tmp$closest)
population <- population[index]
population[is.na(population)] <- 0 #filling in missing points on the path
smoothed.pop <- rep(NA, length(population))
for (i in 2:37){
...
missing.countries = c("Bosnia and Herzegovina", "Central African Republic",
                      ...
                      "Bahamas, The", "Equatorial Guinea", "Laos", "Solomon Islands")
...
replace.countries = c("Bosnia and Herz.", "Central African Rep.", "Congo", "Czech Rep.",
                      ...
                      "Bahamas", "Eq. Guinea", "Lao PDR", "Solomon Is.")
...
Figure S25 shows introduction of MCV2 by mean position and speed of movement for Americas and Africa regions.

```{r, echo=F, fig.height = 6, fig.width = 6}
##' read in the mcv2 introduction data
MCV2_introduction <- read_csv("./data/MCV2_introduction.csv")

##' change column names
colnames(MCV2_introduction) = c("ISO3", "Country", "WHO_Region", "MCV2_Intro_Year", "MCV2_Part_of_country")

##' change intro year to be numeric
MCV2_introduction$MCV2_Intro_Year = as.numeric(MCV2_introduction$MCV2_Intro_Year)

##' only consider AFR and AMR
MCV2.AF.AMR = MCV2_introduction %>% filter(WHO_Region %in% c("AFR", "AMR"))

##' remove coutries which never introduced MCV2
k = which(is.na(MCV2.AF.AMR$MCV2_Intro_Year))
MCV2.AF.AMR = MCV2.AF.AMR[-(k), ]

##' split into separate regions
MCV2.AF = MCV2.AF.AMR %>% filter(WHO_Region == "AFR")
MCV2.AMR = MCV2.AF.AMR %>% filter(WHO_Region == "AMR")

##' find post mcv2 movement. d1 holds the information about which point on the path the country is closest to.
##' The granular version of the path is used for this analysis.
post.mcv2.af = post.mcv2.movements(df.mcv2 = MCV2.AF, d1b = d1b)
post.mcv2.amr = post.mcv2.movements(df.mcv2 = MCV2.AMR, d1b = d1b)


##' calculate averages of movement and position
average.post.mcv2.af = average.movement.since.mcv2(d = post.mcv2.af)
average.post.mcv2.amr = average.movement.since.mcv2(d = post.mcv2.amr)


##' limit plots to a certain number of years post mcv2 introduction
number.years.to.plot = 10
average.post.mcv2.af = average.post.mcv2.af %>% filter(time.since.mcv2 <=number.years.to.plot)
average.post.mcv2.amr = average.post.mcv2.amr %>% filter(time.since.mcv2 <=number.years.to.plot)

##' count the number of countries which have certain number of time points for mcv2 use
number.countries.af = post.mcv2.af %>% group_by(time.since.mcv2) %>% dplyr::summarise(n=n())
number.countries.amr = post.mcv2.amr %>% group_by(time.since.mcv2) %>% dplyr::summarise(n=n())

number.countries.af = number.countries.af %>% filter(time.since.mcv2 > 0, time.since.mcv2 <= number.years.to.plot) %>% 
    dplyr::select(n)

number.countries.amr = number.countries.amr %>% filter(time.since.mcv2 > 0, time.since.mcv2 <= number.years.to.plot) %>% 
    dplyr::select(n)

##' add this to the data frame
average.post.mcv2.af = cbind(average.post.mcv2.af, number.countries.af, "AF")
average.post.mcv2.amr = cbind(average.post.mcv2.amr, number.countries.amr, "AMR")

colnames(average.post.mcv2.af) = c("years", "mean_pos", "median_movement", "lower_quantile",
                                   "upper_quantile", "n", "Region")

colnames(average.post.mcv2.amr) = c("years", "mean_pos", "median_movement", "lower_quantile",
                                   "upper_quantile", "n", "Region")



average.post.mcv2 = rbind(average.post.mcv2.amr, average.post.mcv2.af)

##' do some simple plots
ggplot(average.post.mcv2, aes(x = years, y = mean_pos, col = Region)) + geom_point(aes(size = n)) + theme_light() +
    labs(x = "years post mcv2 ",
         y = "mean position on path",
         size = "country\ncount")



ggplot(average.post.mcv2, aes(x = years, y = median_movement, col = Region)) + geom_point(aes(size = n)) + theme_light() +
    labs(x = "years post mcv2 ",
         y = "median movement on path",
         size = "country\ncount")
