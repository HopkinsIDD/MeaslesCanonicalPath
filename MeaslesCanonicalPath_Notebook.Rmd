...
taxonomy.fig1 = incidence.space.fig(anim.data = canonical.path.data, years = c(1990, 2013), 
...
list[dist.by.age, canonical.path, prop.sus, d1] = 
...
nigeria.pop = subset.pop.by.year %>% filter(Country.Name== "Nigeria") %>% data.frame %>% t
j = which(rownames(nigeria.pop) %in%paste("X", 1981:2014, sep = ""))
nigeria.pop = nigeria.pop[j] %>% data.frame 
...
nigeria.cases = cases.by.country.by.year %>%
    ...
    select(rev(seq(7, ncol(cases.by.country.by.year)-1))) %>%
    as.numeric %>%
    cbind(seq(1981, 2014), .) %>%
    data.frame
...
ggplot(k, aes(x = year, y = weight, col = factor(w))) + geom_point(cex = 3) +
    ...
    scale_colour_manual(values=wes_palette("Darjeeling1")[c(1,5)])
...
logged.scaled.path = awesome.mega.figure.scaled (anim.data = d, years = c(1990, 2013), 
                                     regions = c("AFR",  "AMR"), shapes = c(1,2,16,17),
                                     countries.of.interest = c("Malawi", "United States", 
                                                               "Brazil", "Argentina",
                                                               "Congo, Democratic Republic of the",
...
```{r, echo = include.code, fig.height = 12, fig.width = 10}
...
taxonomy.fig.median = incidence.space.fig.median(anim.data = canonical.path.data, years = c(1990, 2013), 
                                                 regions = c("AFR",  "AMR"), shapes = c(1,2,16,17),
                                                 countries.of.interest = c("Malawi", "United States", 
                                                                           "Brazil", "Argentina",
                                                                           "Congo, Democratic Republic of the",
...
state.space.fig = incidence.space.fig(anim.data = state.space.data, years = c(1990, 2014), 
                                      state.space = 1,
                                     regions = c("AFR",  "AMR"), shapes = c(1,2,16,17),
                                     countries.of.interest = c("Malawi", "United States", 
                                                               "Brazil", "Argentina",
