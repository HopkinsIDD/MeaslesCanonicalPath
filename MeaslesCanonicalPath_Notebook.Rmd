...
dd2 = dd %>% 
  ...
  summarise(mean.pos = weighted.mean(mean.pos, weight))
dd3 = dd %>% 
  group_by(region, year) %>%
  summarise(mean.delta = weighted.mean(mean.delta, weight))
```

```{r, echo = include.code, fig.height = 6, fig.width = 6}

ggplot(dd, aes(x=year, y=mean.pos/2, color=region)) + geom_line(aes(group=country), alpha=0.15) +
  geom_line(data = dd2, lwd=1.2) + theme_minimal() + labs(y="position on path (%)") +
...
ggplot(dd, aes(x=year, y=mean.delta/2, color=region)) + geom_line(aes(group=country), alpha=0.15) +
  geom_line(data = dd3, lwd=1.2) + ylim(-10,10) + theme_minimal() + labs(y="speed of movement on path (% change)") +
  scale_colour_brewer(palette="Dark2", direction=1)
...
for(i in 1:nrow(d1b)){
    ...
    k = d1b %>% filter(Country == c1, Year == (y+1))
    if(nrow(k)>0){
        if(pos < nrow(granular.canonical.path)){
            position.and.movement.comparison[count, ] =  c(c1, k$WHO_REGION, y, 
                                                           ...
                                                           mean(granular.canonical.path$x[pos:(pos+9)], na.rm=T), mean(granular.canonical.path$y[pos:(pos+9)], na.rm=T),
                                                           mean(granular.canonical.path$x[pos:(pos+9)], na.rm=T)-d1b$canonical.x[i],
                                                           mean(granular.canonical.path$y[pos:(pos+9)], na.rm=T)-d1b$canonical.y[i])
...
df$angle.diff10 <- plyr::round_any(df$angle.diff, 10)
...
year.groups <- list(1990:1995, 1996:2000, 2001:2005, 2006:2010, 2011:2017)
all.plots <- list()
for (c in 1:length(regs)){
  for (y in 1:5){
    ...
    for (i in 1:length(byn)){
      ...
      dfa$frequency[which(dfa$obs.exp=="FF")[i]] <- dfd$frequency[match(dfa$a[i], dfd$a)]
      dfa$frequency[which(dfa$obs.exp=="E")[i]] <- dfe$frequency[match(dfa$a[i], dfe$a)]
    }
    dfa$label <- c(rep("Observed",length(byn)), rep("Difference",length(byn)), rep("Expected",length(byn)))
    dfa$frequency[is.na(dfa$frequency)] <- 0
    ...
    all.plots[[name]] <- 
      ...
      scale_fill_distiller(palette="YlGnBu", direction=1, limits=c(0,0.2)) + #Greys
...
#only plot 2000-2017 in the main figure, all years will be in the supplement
all.plots[[3]]; all.plots[[4]]; all.plots[[5]]; all.plots[[8]]; all.plots[[9]]; all.plots[[10]]; all.plots[[13]]; all.plots[[14]]; all.plots[[15]]; all.plots[[18]]; all.plots[[19]]; all.plots[[20]]; all.plots[[23]]; all.plots[[24]]; all.plots[[25]]; all.plots[[28]]; all.plots[[29]]; all.plots[[30]]
```
...
```{r, echo = include.code, fig.height = 96, fig.width = 18}

##' make colour palette
colfunc = colorRampPalette(c("green", "white", "orange"))

##' plot AFRO
par(mfrow=c(16,3), bty="l", mar=c(8, 10, 8, 2) + 0.1)
...
for (c in 1:length(uncodes.afro)){
  plot(0, bg = colfunc(28), col = 'black', pch = '', bty="n", ylab="incidence per 100,000", xlab="coefficent of variation", yaxt="n",
       xlim = xlim, ylim = ylim, cex.lab=2, cex.axis=2, mgp=c(4, 1.5, 0))
  axis(2, log10(c(1,50,100,250,500,1000,2000,4000)), labels=F, cex.lab=2)
  text(y=log10(c(1,50,100,250,500,1000,2000,4000)), par("usr")[1] - 0.05, 
       labels = c(1,50,100,250,500,1000,2000,4000), srt = 45, pos = 2, xpd = TRUE, cex=2)
  title(country.names.afro[c], cex.main=3)
  ...
  for(i in 1:100){
    points(cvs.afro[((100*c)-(100-i)), 2:ncol(cvs.afro)],
           ...
           bg = colfunc(28), col = 'black', pch = 21, cex=2)
  }
  lines(colMeans(cvs.afro[(100*c-99):(100*c), 2:ncol(cvs.afro)]), 
        colMeans(incs.afro[(100*c-99):(100*c), 2:ncol(incs.afro)]), col = 'black', lwd=4)
  points(Af.cv.ss[c,], incs.afro.ss[c,], pch=3, bg = colfunc(28), lwd=3)
  lines(Af.cv.ss[c,], incs.afro.ss[c,], col="grey", lwd=4)
  LEGEND("topright", c("simulated", "observed"), pch=c(21,3), line.col=c("black","grey"), pt.col="black",lty=1, cex=2, lwd=3)
}
...
```{r, echo = include.code, fig.height = 66, fig.width = 18}

##' make colour palette
colfunc = colorRampPalette(c("green", "white", "orange"))

par(mfrow=c(11,3), bty="l", mar=c(8, 10, 8, 2) + 0.1)
ylim = c(-0.5, as.numeric(max(log10(as.numeric(incs[, 2:ncol(incs)])*100000), na.rm = T)))
xlim = c(0,4)
for (c in 1:length(uncodes.amro)){
  plot(0, bg = colfunc(28), col = 'black', pch = '', bty="n", ylab="incidence per 100,000", xlab="coefficent of variation", yaxt="n",
  ...
  text(y=log10(c(1,50,100,250,500,1000,2000,4000)), par("usr")[1] - 0.05, 
       labels = c(1,50,100,250,500,1000,2000,4000), srt = 45, pos = 2, xpd = TRUE, cex=2)
  title(country.names.amro[c], cex.main=3)
  ...
  lines(colMeans(cvs.amro[(100*c-99):(100*c), 2:ncol(cvs.amro)]), 
        colMeans(incs.amro[(100*c-99):(100*c), 2:ncol(incs.amro)]), col = 'black', lwd=4)
  points(Am.cv.ss[c,], incs.amro.ss[c,], pch=3, bg = colfunc(28), cex=3, lwd=3)
  lines(Am.cv.ss[c,], incs.amro.ss[c,], col="grey", lwd=4)
  LEGEND("topright", c("simulated", "observed"), pch=c(21,3), line.col=c("black","grey"), pt.col="black",lty=1, cex=2, lwd=3)
}
...
Figure S9 displays the full results from the direction of movement analysis for years 1990-2017 across the columns and all regions of the world across the rows. Similar to Fig. 4C in the main text, we display the frequency of expected and observed movements, and the difference between them, for each region over 5, 6, or 7 years.  The heat map captures the frequency of countries movement in a direction grouped by 10 degree angle groups.

```{r, fig.height = 4.6, fig.width = 10}
all.plots
```

Figures S10 and S11, contains the results of modeling incidence and coefficient of variation as dependent variables separately, with birth rate, vacciation proportion and (birth rate) times (1-vacciantion proportion) all used independent variables (one at a time) using generalized additivie models (GAM). The term (birth rate) times (1-vaccination proportion) is an approximation of how quickly individuals who are susceptible to measles are recruited into the population, therefore we term this the rate of susceptible recruitment. To produce the GAM plots, we first need to construct a data set which has the estimated mean age of susceptibles by country and by year.
...
We can then plot these predicted paths in the incidence-space over values of birth rate, vaccination rate, and the rate of susceptible recruitment with reported cases (Fig. S10).
...
Figure S10C also displays the association between incidence or coefficient of variation with the birth rate and vaccination coverage in Africa.
...
Similarly, we can use model incidence and coefficient of variation using GAMS, as seen above for the reported case data. This is seen below and Fig S11 of the paper.
...
Figure S12 shows the age distribution of cases in Malawi and Angola from 2006-2013. This figure is reproduced below.
