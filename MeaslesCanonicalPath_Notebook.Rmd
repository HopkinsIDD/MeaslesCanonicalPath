...
We cam easily pull out individual countries postion and speed of movement along the canonical path. Here we demonstrates movement along the canonical path for 10 countries of interest from the Americas, Africa, and European (supplemental figure S20).
...
Figure S4 of the supplement has the location of countries in the WHO Africa and Americas Regions in incidence-space in 1990 and 2014 post scaling of incidence and CV. This is re-created here. The dotted line represents movement between 2015 and 2017.
...
list[anim.data, Af.data, Amr.data, Rest.data] = 
    prepare.anim.data.for.analysis(input.data = canonical.path.data.reported, #using canonical path based on reported data again
                                   mean.age.sus = mean.age.sus)
...
We will start with the square root incidence and then scaling both square root incidence and coefficent of variation so that all of their values are between 0 and 1. We are using reported cases again, so we will need to make sure to set use.reported.cases to TRUE

```{r, echo = include.code, include = FALSE}

##' square root incidence
d = canonical.path.data.reported
use.reported.cases = TRUE
regions = c("EMR","EUR","AFR","AMR","WPR","SEAR")
make.inc.cv.scale.same = 1 
sqrt.inc = T
