sia.data<-read.csv("state.space.model/input-SIA data with population numbers.csv",header=T)

cases<-read.csv("state.space.model/input-cases.csv",header=T,row.names=1)
...
write.table(sia.coverage,file="state.space.model/output-sia_coverage.csv",sep=",",row.names=rownames(cases),col.names=1980:(1979+dim(sia.coverage)[2]))
write.table(sia.effective,file="state.space.model/output-sia_effective.csv",sep=",",row.names=rownames(cases),col.names=1980:(1979+dim(sia.coverage)[2]))
write.table(sia.new,file="state.space.model/output-sia_age_corrected.csv",sep=",",row.names=rownames(cases),col.names=1980:(1979+dim(sia.coverage)[2]))
